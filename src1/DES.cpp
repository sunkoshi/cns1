...
string convertAsciiToBinary(string str){

    string binary = "";
    for (int i = 0; i < str.length(); i++){
        char c= str[i];
        string temp = bitset<8>(c).to_string();
        binary += temp;
    }
    return binary;
}
...
string DES(string pt, vector<string> round_keys) {
...
int main() {
    ...
    string pt;
    cin>>pt;
    pt = convertAsciiToBinary(pt);

    vector<string> round_keys = generate_keys(key);
    string ciphertext = DES(pt, round_keys);



    cout << "The plain text is: " << pt << endl;
    cout << "The cipher text is: " << ciphertext << endl;


    ///Now decrypting the cipher text
    //reverse the round keys
    reverse(round_keys.begin(), round_keys.end());

    string decrypted_text = DES(ciphertext, round_keys);

    //converting the binary to ascii
    cout << "The decrypted text is: " << convertBinaryToAscii(decrypted_text) << endl;
